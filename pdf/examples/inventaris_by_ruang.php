...
$pdf->AddPage();
$html ='<b>Rekapitulasi Inventaris Per Ruangan</b>';
$lokasi=$_GET['lok'];
if($_SESSION['level']==1) {
    $sql1="select barang_detail.*,barang.*,nama_panjang,COUNT(IF(kondisi = 'Baik', 1, NULL)) as qty_baik,COUNT(IF(kondisi = 'Rusak', 1, NULL)) as qty_rusak from barang_detail,barang,unit_kerja where barang_detail.id_barang=barang.id_barang and unit_kerja.id_unit=barang_detail.id_unitkerja and md5(lokasi)='$lokasi' GROUP BY barang_detail.id_unitkerja,lokasi";
  } else {
    //$id_unit= $_SESSION['idunit'];
    $sql1="select barang_detail.*,barang.*,nama_panjang,COUNT(IF(kondisi = 'Baik', 1, NULL)) as qty_baik,COUNT(IF(kondisi = 'Rusak', 1, NULL)) as qty_rusak from barang_detail,barang,unit_kerja where barang_detail.id_barang=barang.id_barang and unit_kerja.id_unit=barang_detail.id_unitkerja and barang_detail.id_unitkerja=$id_unit and md5(lokasi)='$lokasi' GROUP BY barang_detail.id_unitkerja,lokasi";
  }
