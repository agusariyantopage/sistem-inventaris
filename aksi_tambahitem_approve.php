<?php
	...
	//Mengambil Status Pengajuan
	$sql0="select * from tambah where id_tambah=$id";
	$perintah0=mysqli_query($koneksi,$sql0);
	$r0=mysqli_fetch_array($perintah0);
	$jenis=$r0['tipe_sumber'];
	if($jenis=='Bantuan'){
		$trm=1;
	} else {
		$trm=0;
	...
	$sql1="update tambah set status ='Disetujui',tgl_setuju='$tanggal',update_terakhir=Default,diterima=$trm where id_tambah=$id";
	...
	// Tambah Data Barang Ke Inventaris Jika Jenis = Bantuan
	if($trm==1){
